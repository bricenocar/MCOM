...
async function run() {
    try {
        ...
        execSync(`node ./dist/capture ${sourceManagementApiEndpoint} "${sourceManagementApiAccessToken}" "${snapshotFolder}"`);

        // remove all content of the target portal
        execSync(`node ./dist/cleanup ${destManagementApiEndpoint} "${destManagementApiAccessToken}"`);

        // upload the content of the source portal
        execSync(`node ./dist/generate ${destManagementApiEndpoint} "${destManagementApiAccessToken}" "${snapshotFolder}"`);
