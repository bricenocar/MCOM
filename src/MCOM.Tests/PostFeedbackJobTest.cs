...
namespace MCOM.Tests
{
    public class PostFeedbackJobTest
    {
        ...
        public async Task When_The_Function_Runs_Ok(int itemCount)
        {
            ...
            while (i < itemCount)
            {
                var strQueueItem = JsonConvert.SerializeObject(new QueueItem()
                {
                    ...
                    Item = new FeedbackItem() { DocumentId = $"DocumentId-{i}", DriveId = $"DriveId-{i}" }
                });
            ...
            var mockQueueService = MockConfiguration(messages, response);

            // Build azure function
            var postFeedbackJob = new PostFeedbackJob(mockQueueService.Object);
        ...
        /// <returns></returns>
        private static Mock<IQueueService> MockConfiguration(List<CloudQueueMessage> messages, HttpResponseMessage responseMessage)
        {
            // Mock variables
